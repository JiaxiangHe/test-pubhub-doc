...
When a user authenticates, they receive an authorization token to include in the request of each API operation. Provide the API token on every request using the `Authorization` request header with a value of `Bearer <api token>`. For example:
